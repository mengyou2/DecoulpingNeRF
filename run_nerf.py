...
def config_parser():
    ...
    parser.add_argument("--surface_loss_lambda", type=float, default=0.01,
                        help='lambda of surface loss')
    parser.add_argument("--warp_loss_lambda", type=float, default=1.0,
                        help='lambda of warp loss')   
...
def train():
    ...
    if args.pretrain:
        ...
        if args.ft_path_S is not None and args.ft_path_S != 'None':
            ...
            for i in range(args.N_iters):
                ...
                coords_s = torch.stack((torch.where(mask == 1.)), -1)
                ...
                global_step += 1
        if not os.path.exists(os.path.join(basedir, expname, 'pretain')):
            os.makedirs(os.path.join(basedir, expname, 'pretain'))
        # Save the pretrained weight
        torch.save({
            'global_step': global_step,
            'network_fn_s_state_dict': render_kwargs_train['network_fn_s'].state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
        }, os.path.join(basedir, expname, 'pretain','Pretrained_S.tar'))
    ...
    for i in range(start, args.N_iters):
        ...
        rays_o_all, rays_d_all = get_rays(H, W, focal, torch.Tensor(pose)) # (H, W, 3), (H, W, 3)
        coords_d = torch.stack((torch.where(mask < 1.)), -1)
        coords_s = torch.stack((torch.where(mask == 1.)), -1)
        ...
        rays_o = select_batch(rays_o_all) # (N_rand, 3)
        rays_d = select_batch(rays_d_all) # (N_rand, 3)
        target_rgb = select_batch(target)
        ...
        #random select patch
        patch_s = 64
        h_s = np.random.randint(low=0, high=H-patch_s)
        w_s = np.random.randint(low=0, high=W-patch_s)
        h_inds = np.arange(h_s,h_s+patch_s)
        w_inds = np.arange(w_s,w_s+patch_s)
        inds = np.repeat(np.expand_dims(np.transpose(h_inds * W),axis=1),patch_s,axis=1)+w_inds
        select_coords_loss = coords[inds]
        select_coords_loss = select_coords_loss.view(int(patch_s**2),2)
        
        #random choose pose
        img_rand = np.random.choice([x for x in i_train if x != img_i])
        pose_rand = poses[img_rand, :3, :4]
        rays_o_rand, rays_d_rand = get_rays(H, W, focal, torch.Tensor(pose_rand)) # (H, W, 3), (H, W, 3)
        rays_o_rand = select_batch(rays_o_rand,select_coords_loss) # (N_rand, 3)
        rays_d_rand = select_batch(rays_d_rand,select_coords_loss) # (N_rand, 3)
        batch_rays_rand = torch.stack([rays_o_rand, rays_d_rand], 0)
        rays_o_curr = select_batch(rays_o_all,select_coords_loss) # (N_rand, 3)
        rays_d_curr = select_batch(rays_d_all,select_coords_loss) # (N_rand, 3)
        batch_rays_curr = torch.stack([rays_o_curr, rays_d_curr], 0)
        with torch.no_grad():
            ret_rand = render(t,
                        chain_5frames,
                        H, W, focal,
                        chunk=args.chunk,
                        rays=batch_rays_rand,
                        **render_kwargs_train)
            ret_curr = render(t,
                        chain_5frames,
                        H, W, focal,
                        chunk=args.chunk,
                        rays=batch_rays_curr,
                        **render_kwargs_train)
        
        
        warp_loss_1,warp_1 = warp_comparsion(ret_curr['rgb_map_d'],ret_rand['rgb_map_d'],ret_curr['depth_map_d'],pose,pose_rand,int(patch_s),int(patch_s),focal) 
        warp_loss_2,warp_2 = warp_comparsion(ret_curr['rgb_map_full'],ret_rand['rgb_map_full'],ret_curr['depth_map_full'],pose,pose_rand,int(patch_s),int(patch_s),focal) 
        warp_loss = warp_loss_1+warp_loss_2
        loss_dict['warp_loss'] = warp_loss
        loss += args.warp_loss_lambda * loss_dict['warp_loss']
        ...
        surface_loss = L1(ret['surf_raw_b']-ret['surf_b']) \
                            +  L1(ret['surf_raw_f']-ret['surf_f'])
        loss += args.surface_loss_lambda *surface_loss
        loss_dict['surface_loss'] = surface_loss
        ...
        if i % args.i_img == 0:
            ...
            with torch.no_grad():
                ...
                writer.add_image("patch_rgb_rand", torch.clamp(ret_rand['rgb_map_d'].reshape(int(patch_s),int(patch_s),3), 0., 1.), global_step=i, dataformats='HWC')
                writer.add_image("patch_rgb_rand", torch.clamp(ret_rand['rgb_map_full'].reshape(int(patch_s),int(patch_s),3), 0., 1.), global_step=i, dataformats='HWC')
                writer.add_image("patch_rgb_curr", torch.clamp(ret_curr['rgb_map_d'].reshape(int(patch_s),int(patch_s),3), 0., 1.), global_step=i, dataformats='HWC')
                writer.add_image("patch_rgb_curr", torch.clamp(ret_curr['rgb_map_full'].reshape(int(patch_s),int(patch_s),3), 0., 1.), global_step=i, dataformats='HWC')


                writer.add_image("warp_d", torch.clamp(warp_1[0].permute(1,2,0), 0., 1.), global_step=i, dataformats='HWC')
                writer.add_image("warp_f", torch.clamp(warp_2[0].permute(1,2,0), 0., 1.), global_step=i, dataformats='HWC')
