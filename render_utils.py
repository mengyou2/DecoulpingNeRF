...
def render_path_only(render_poses,
                time2render,
                hwf,
                chunk,
                render_kwargs,
    ...
    for i, c2w in enumerate(render_poses):
        ...
        # print(ret['dynamicness_map'].max(),ret['dynamicness_map'].max())
...
# 
        ...
        mask_np = ret['dynamicness_map'][:, :, None] 
        mask_np[mask_np>=0.5] = 1
        mask_np[mask_np<0.5] = 0
        induced_flow_f = np.multiply(induced_flow_f, mask_np.cpu().numpy())
        print(induced_flow_f.max())

        induced_flow_f_img = flow_to_image(induced_flow_f,global_max=15)
        ...
        induced_flow_b = np.multiply(induced_flow_b, mask_np.cpu().numpy())
        print(induced_flow_b.max())
        induced_flow_b_img = flow_to_image(induced_flow_b,global_max=15)
        ...
        if savedir is not None:
            ...
            rgb8s = to8b(rgbs_s[-1])
            filename = os.path.join(savedir, 's_{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8s)

            depth8 = to8b(depths[-1])
            filename = os.path.join(savedir, 'd_{:03d}.png'.format(i))
            imageio.imwrite(filename, depth8)
            
            dynamicness8 = to8b(dynamicness[-1])
            filename = os.path.join(savedir, 'dynamicness_{:03d}.png'.format(i))
            imageio.imwrite(filename, dynamicness8)

            
            ff8 = flows_f[-1]
            filename = os.path.join(savedir, 'ff_{:03d}.png'.format(i))
            imageio.imwrite(filename, ff8)

            fb8 = flows_b[-1]
            filename = os.path.join(savedir, 'fb_{:03d}.png'.format(i))
            imageio.imwrite(filename, fb8)

            mask = (dynamicness8==0)
            ff_bd = ff8*(1-mask[:,:,None])
            ff_bd[mask]=255
            filename = os.path.join(savedir, 'ff_bd1_{:03d}.png'.format(i))
            imageio.imwrite(filename, ff_bd)
            fb_bd = fb8*(1-mask[:,:,None])
            fb_bd[mask]=255
            filename = os.path.join(savedir, 'fb_bd1_{:03d}.png'.format(i))
            imageio.imwrite(filename, fb_bd)
...
def render_path(render_poses,
    ...
    for i, c2w in enumerate(render_poses):
        ...
        induced_flow_f_ = induce_flow(H, W, focal, pose_f, ret['weights_d'], ret['raw_pts_f'], grid[..., :2])
        induced_flow_b_ = induce_flow(H, W, focal, pose_b, ret['weights_d'], ret['raw_pts_b'], grid[..., :2])
        # induced_flow_f_ = induce_flow(H, W, focal, pose_f, ret['weights_d'], ret['raw_pts_f']*ret['blending'].detach()[..., None], grid[..., :2])
        # induced_flow_b_ = induce_flow(H, W, focal, pose_b, ret['weights_d'], ret['raw_pts_b']*ret['blending'].detach()[..., None], grid[..., :2])
        # print(tt)
# 
        if (i + 1) >= len(render_poses):
            induced_flow_f = np.zeros((H, W, 2))
...
def render_rays(t,
    ...
    with torch.no_grad():
        surf = torch.sum(weights_d.unsqueeze(2) * pts,1)
        surf = surf.unsqueeze(1)
        surf_ref = torch.cat([surf, torch.ones_like(surf[..., 0:1]) * (t - t_interval)], -1)
        raw_surf = network_query_fn_d(surf_ref, viewdirs, network_fn_d)
        surf_raw_b = surf + raw_surf[:,:,4:7]
        surf_raw_f = surf + raw_surf[:,:,7:10]
    ret['surf_raw_f'] = surf_raw_f
    ret['surf_raw_b'] = surf_raw_b
    # Rerndering t - 1
    rgb_map_d_b, weights_d_b = raw2outputs_d(raw_d_b_rgba,
                                             z_vals,
                                             rays_d,
                                             raw_noise_std)
    surf_b = torch.sum(weights_d_b.unsqueeze(2) * pts_b[..., :3],1)
    ...
    rgb_map_d_f, weights_d_f = raw2outputs_d(raw_d_f_rgba,
                                             ...
                                             rays_d,
                                             raw_noise_std)
    surf_f = torch.sum(weights_d_f.unsqueeze(2) * pts_f[..., :3],1)
    # print(surf_f.shape)
